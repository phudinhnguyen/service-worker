pipeline {
    ...
    environment {
        ...
        HOST = '192.168.10.5'
        CREDENTIAL_NAME = 'alta-pc'
        SERVER_WORKING_DIR = '/data/test'
        LOCAL_BUILD_DIR = 'dist'
        STATIC_DEPLOY_DIR = 'frontend'
        IS_BACKUP = 'true'
        BACKUP_DEPLOY_DIR = 'FrontendBackup'
                    ...
                    steps {
                        script {
                            if (env.BRANCH_NAME == 'develop' ){
                                ...
                                Deploy("${CREDENTIAL_NAME}","${HOST}","${DEPLOY_FILENAME}","${SERVER_WORKING_DIR}","${LOCAL_BUILD_DIR}","${STATIC_DEPLOY_DIR}","${BACKUP_DEPLOY_DIR}","${IS_BACKUP}","${MAX_BACKUP_VERSION}","${VERSION_FILENAME}")
                            }
...
def Deploy(CREDENTIAL_NAME, HOST, SH_FILENAME, SERVER_WORKING_DIR, LOCAL_BUILD_DIR, STATIC_DEPLOY_DIR, BACKUP_DEPLOY_DIR, IS_BACKUP, MAX_BACKUP_VERSION, VERSION_FILENAME){
