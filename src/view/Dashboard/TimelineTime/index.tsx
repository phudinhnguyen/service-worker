...
const TimelineTime = props => {
    ...
    const { currentMonth, device } = useSelector((state: RootState) => state.timeCalendar)
    const [ getCalendarOfTimeByDay ] = useAsync(schedulePresenter.getCalendarOfTimeByDay)
    const { listDayInMonth } = getTotalDayInMonth(currentMonth)

    useEffect(() => {
        getCalendarOfTimeByDay.execute({ monthMoment: moment(activeDay), deviceId })
    }, [ activeDay ])
    ...
    return <div className="dashboard">
        ...
        <div className="device-schedule">
                    ...
                    <div className="list-day-in-monh">
                        {
                            listDayInMonth.map(day => {
                                const isActiveDay = (() => {
                                    ...
                                    return moment(activeDay, "YYYY-MM-DD").isSame(moment(`${ currentMonth.year() }-${ currentMonth.month() + 1 }-${ day }`, "YYYY-M-DD"))
                                })()

                                return <div className="day" style={{ backgroundColor: isActiveDay && "#FF9911" }}>
                                    {day}
                                </div>
                            })
                        }
            ...
            <div className="list-device-by-calendar" style={{ opacity: getCalendarOfTimeByDay.status == "loading" ? ".5" : 1 }}>
                <ListTimeInDay />
                <Calendar />
                {
